...
description: "Šiame straipsnyje apžvelgiami gamybos procesai. Jame aprašoma įvairių etapų gamybos užsakymų, partijos pavedimus ir kanbans, nuo užsakymo sukūrimo iki uždarymo finansiniam laikotarpiui."
author: YuyuScheller
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
Šiame straipsnyje apžvelgiami gamybos procesai. Jame aprašoma įvairių etapų gamybos užsakymų, partijos pavedimus ir kanbans, nuo užsakymo sukūrimo iki uždarymo finansiniam laikotarpiui. 
...
Į **gamybos kontrole** modulis yra susijęs su kitais moduliais, pavyzdžiui, **produkto informacijos valdymo**, **atsargų valdymo**, **DK**, **sandėlio valdymas**, **projekto apskaitos**, ir **organizacijos administracijos**. Ši integracija palaiko informacijos srautą, reikalingą galutinei prekei pagaminti.  

Gamybos procesui paprastai įtaką daro išlaidų apskaitos ir atsargų vertinimo būdai, kurie pasirenkami konkrečiam gamybos procesui. Dinamika 365 operacijoms palaiko tiek faktines išlaidas (pirmasis, pirmoji iš \[FIFO\], Paskutinė, pirmą kartą, \[LIFO\]; slenkantis vidurkis ir periodiškai svertinį vidurkį) ir standarto kaina metodus. „Lean manufacturing“ įdiegiamas pagal įkainojimo atvirkštine tvarka principą.  
...
Įvairūs produktai ir gamybos topologijos reikalauja taikyti skirtingus užsakymo tipus. Dinamika 365 operacijoms galite taikyti įvairių užsakymų tipus in mišriu režimu. Kitaip tariant, gaminant vieną baigtą produktą iki galo, proceso metu galimi visi užsakymo tipai.
...
2.  **Įvertinta** – galite apskaičiuoti medžiagų ir išteklių suvartojimo įvertinimus. Įvertinimas generuoja žaliavų, kurių būsena yra **Užsakyta**, atsargų operacijas. Pajamas už pagrindinių produktų, bendrų produktų ir šalutinių produktų yra kurti kai gamybos užsakymai arba partijos užsakymus apskaičiuota. Jei KS yra eilutės, **susietas tiekimo** tipo, medžiagos, pirkimo užsakymus arba subrangovo eksploatavimo paslaugos yra sukurtas ir susietas su gamybos užsakymo ar partijos tvarka. Prekės ar užsakymai rezervuojami pagal gamybos užsakymo rezervavimo strategiją, o pagamintų prekių kaina skaičiuojama pagal parametrų nuostatas.
...
5.  **Paruošti**/**paimta** – visos medžiagos ir ištekliai buvo suskirstytas į gamybos vietą, KS eilutes arba kanban eilutės atnaujinamos į statusas **paimta**. Iškviesto tiekimo užsakymai ir susijęs sandėlio darbas šiame etape paprastai būna atlikti. „Kanban“ korteles arba užduočių korteles, reikalingas pateikti gamybos eigos ataskaitą, reikia priskirti ir spausdinti.
