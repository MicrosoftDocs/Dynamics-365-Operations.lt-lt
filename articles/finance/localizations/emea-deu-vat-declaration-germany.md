...
description: Šioje temoje aprašoma, kaip nustatyti ir generuoti Vokietijos išankstinę pridėtinės vertės mokesčio (PVM) deklaraciją oficialiu XML formatu.
author: anasyash
ms.date: 11/22/2021
ms.topic: article
...
ms.locfileid: "7860869"
---
# <a name="vat-declaration-germany"></a>PVM deklaracija (Vokietija)

[!include [banner](../includes/banner.md)]

Šioje temoje aprašoma, kaip nustatyti ir generuoti Vokietijos išankstinę pridėtinės vertės mokesčio (PVM) deklaraciją oficialiu XML formatu. Šioje temoje taip pat paaiškinama, kaip peržiūrėti PVM deklaraciją Microsoft Excel.
...
Vokietijoje turite konfigūruoti lauko **Ataskaitos peržvalgą**. Daugiau informacijos apie tai, kaip nustatyti specialius programos parametrus, ieškokite šios temos skyriuje Nustatyti programai brangius PVM [deklaravimo](#set-up-application-specific-parameters-for-vat-declaration-fields) laukų parametrus.
...
Kitaip, galite konfigūruoti du atskirus PVM kodus: vieną mokėtinam PVM ir kitą PVM atskaitymui. Tada susiekite kiekvieną kodą su atitinkamais ataskaitos lauko **peržvalgos** rezultatais.

Pavyzdžiui, apmokestinamų įsigijimų bendrijos viduje standartiniu tarifu atveju konfigūruojate PVM kodą UT_S_EU naudojimo mokesčiu ir susiejate jį **su** **34-UseTaxEUPurchaseStandard peržvalgos rezultatu Ataskaitos lauko** **peržvalga.** Šiuo atveju sumos, kurios naudoja UT_S_EU PVM kodą, rodomos laukuose **089** ir 061 (34 ir 56 eilutės).

Taip pat konfigūruojate du PVM kodus:

  - **VAT_S_EU,** kurios mokesčių tarifo vertė -19 procentų
  - **InVAT_S_EU,** kurios mokesčių tarifo vertė yra 19 procentų
...
Norėdami gauti daugiau informacijos apie tai, kaip konfigūruoti atvirkštinį PVM, žr. [Atvirkštinį](emea-reverse-charge.md) apmokestinimą.
...
Daugiau informacijos apie tai, kaip nustatyti registracijos kategorijas ir registracijos tipus, [ieškokite registracijos](emea-registration-ids.md) ID.
...
4. Sąlygų **FastTab** nustatykite šiuos laukus, norėdami susieti PVM kodus ir ataskaitos laukus.
    ...
    | Laukas                  | Aprašymas                                                                                                                                                                                                                                                                                                          |
    |------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    | Paieškos rezultatas          | Pasirinkti ataskaitos lauko vertę. Daugiau informacijos apie vertes ir jų priskyrimą PVM deklaracijos eilutėms žr. [anksčiau šios temos PVM deklaracijos apžvalgos](#vat-declaration-overview) skyriuje.                                                                                               |
...
7. Pasirinkite PVM deklaracijos Excel (DE) konfigūraciją, tada veiksmų srityje pasirinkite Importuoti, norėdami importuoti parametrus, kuriuos sukonfigūravote **PVM** deklaracijos **XML** **(DE)** formatui.
...
2. Eikite **į DK nustatymą DK** > **·** > **parametrai**.
...
Jei konfigūruojate PVM deklaraciją juridiniame subjekte, kuriame yra [kelios PVM](emea-reporting-for-multiple-vat-registrations.md) registracijos, atlikite šiuos veiksmus:

1. Eikite **į DK nustatymą DK** > **·** > **parametrai**.
...
Duomenų pakete yra elektroninio pranešimo parametrai, naudojami PVM deklaracijai sugeneruoti XML formatu, o tada peržiūrėti programoje "Excel". Galite išplėsti šiuos parametrus arba sukurti savo. Norėdami gauti daugiau informacijos apie tai, kaip dirbti su elektroniniais pranešimais ir kurti savo parametrus, [žr. Elektroninių pranešimų siuntimo](../general-ledger/electronic-messaging.md) informaciją.

1. Ciklo [Microsoft Dynamics tarnybose (LCS) bendrai naudojamų turtų bibliotekoje pasirinkite Duomenų paketas kaip turto tipą, tada atsisiųskite](https://lcs.dynamics.com/v2) DE VAT deklaracijos **EM** **paketą**. Atsisiųsto failo vardas yra **DE VAT deklaracijos EM package.zip.**
2. Duomenų Dynamics 365 Finance valdymo darbo srityje **pasirinkite** **Importuoti**.
...
3. PVM **mokėjimo versijos lauke** pasirinkite vieną iš šių verčių:
    ...
    - **Pradinis:** sugeneruokite pradinio PVM mokėjimo PVM operacijų ataskaitą arba prieš sugeneruodami PVM mokėjimą.
...
2. Pasirinkite **Spausdinti** ataskaitą, tada pasirinkite **Gerai**.
...
Kai naudojate elektroninius pranešimus ataskaitai generuoti, galite rinkti mokesčių duomenis iš kelių juridinių subjektų. Norėdami gauti daugiau informacijos, toliau [šioje temoje žr. skyrių Paleisti kelių](#run-a-vat-declaration-for-multiple-legal-entities) juridinių subjektų PVM deklaraciją.
...
8. Pranešimų **·** "FastTab" pasirinkite **Naujinti** būseną. Būsenos **atnaujinimo dialogo** lange pasirinkite Parengta **generuoti**, tada pasirinkite **Gerai**. Patikrinkite, ar pranešimo būsena pakeista į Parengta **generuoti**.
...
4. Veiksmų puslapyje **Automatiškai įvesti** įrašus pasirinkite eilutę, kur bus automatiškai **įvedami PVM grąžinimo** įrašai.
...
5. Duomenų **šaltinių nustatymo tinklelyje** pridėkite kiekvieno papildomo juridinio subjekto, kuris turi būti įtrauktas į ataskaitą, eilutę. Kiekvienai naujai eilutei nustatykite šiuos laukus.
    ...
    | Laukas                  | Aprašymas                                                                                                                   |
    |------------------------|-------------------------------------------------------------------------------------------------------------------------------|
    | Pavadinimas                   | Įveskite vertę, kuri padės suprasti, iš kur gaunamas šis įrašas. Pavyzdžiui, įveskite **filialo 1 PVM** mokėjimą. |
    | Pranešimo prekės tipas      | Pasirinkite **PVM** grąžinimą. Ši vertė yra vienintelė galima visų įrašų vertė.                                    |
