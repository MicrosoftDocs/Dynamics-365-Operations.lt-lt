...
Šioje temoje pateikiama informacija, kuri padės jums pradėti nuo Italijos finansų ir tiekimo grandinės valdymo elektroninių SF išrašymo. Tai padės jums atlikti konfigūravimo veiksmus, kurie priklauso nuo šalies / regiono kontrolės konfigūracijos tarnybose (RCS). Šie veiksmai papildo veiksmus, aprašytus skyriuje [Darbo pradžia naudojant elektroninių SF išrašymą](e-invoicing-get-started.md).

## <a name="prerequisites"></a>Būtinieji komponentai

Prieš pradėdami šioje temoje aprašytus veiksmus, turite įvykdyti šiuos būtinuosius komponentus:

- Atlikite veiksmus darbo [su elektroninių SF išrašymu pradžia](e-invoicing-get-started.md).
- Importuoti Italijos **EsturaPA (IT) elektroninių** SF išrašymo priemonę į RCS iš visuotinės saugyklos. Norėdami gauti daugiau informacijos, žr. anksčiau nurodytos temos "Pradėti naudoti elektroninių SF išrašymą" Microsoft konfigūracijos teikėjo skyriaus elektroninių SF [išrašymo](e-invoicing-get-started.md#import-an-electronic-invoicing-feature-from-the-microsoft-configuration-provider) priemonę.
- Įtraukite saitus iš reikalingų sertifikatų į aptarnavimo aplinką. Reikiami sertifikatai apima skaitmeninio parašo sertifikatą, sertifikatų tarnybos (CA) sertifikatą ir kliento programų sertifikatą. Daugiau informacijos ieškokite temos ["Darbo su elektroninio SF išrašymo paslauga" skaitmeninio sertifikato slapto](e-invoicing-get-started-service-administration.md#create-a-digital-certificate-secret) skyriaus kūrimas.
...
Šiame skyriuje papildoma temos "Pradėti naudoti elektroninę SF išrašymą" konkrečios šalies programos [nustatymo](e-invoicing-get-started.md#country-specific-configuration-of-application-setup) skyriaus konfigūracija.

### <a name="create-a-new-feature"></a>Sukurti naują funkciją
...
1. Elektroninio **SF išrašymo priemonių** puslapyje pasirinkite norimą redaguoti priemonę.
2. Skirtuke **Nustatymai pasirinkite Pardavimo** **SF**, tada – **Redaguoti**.
...
1. Užbaikite, publikuokite ir įdiekite funkciją tarnybos aplinkoje. Norėdami gauti daugiau informacijos, žr. temos "Pradėti naudoti elektroninio SF išrašymą" skyriuje Elektroninių SF išrašymo funkcija paslaugų [aplinkoje](e-invoicing-get-started.md#deploy-the-electronic-invoicing-feature-to-service-environment).
2. Įdiekite funkciją į prijungtą programą. Daugiau informacijos ieškokite temos "Pradėti naudoti elektroninį SF išrašymą" skyriuje "Pradėti nuo elektroninių SF išrašymo" skyrių Elektroninio SF [išrašymo](e-invoicing-get-started.md#deploy-the-electronic-invoicing-feature-to-connected-application) funkcija.
...
3. Pasirinkite **saugyklose** \> **atidaromi** \> **visuotiniai** atidaryti.
...
7. Skirtuke Elektroninis dokumentas įsitikinkite, kad kliento SF žurnalo ir projekto SF laukai nustatyti pagal **informaciją**, **·** **·**[konfigūravimo programos](e-invoicing-get-started.md#configure-the-application-setup) nustatymą.
...
3. Pasirinkite **Kliento SF konteksto** modelis, tada pasirinkite Kurti **konfigūraciją**.
...
7. Aprašų **medyje** pasirinkite **DataChannel, tada** pasirinkite Konstruktorius. **·**
8. Duomenų **šaltinių** medyje išplėskite kontekstinio **\$ kanalo \_ konteinerį.**
...
3. Prisiregistruokite prie ["Azure"](https://portal.azure.com) portalo kaip administratorius.
4. Sukurkite SDI tarpinio serverio tarnybos programos registraciją.
    ...
    4. Pereikite prie Sertifikatų > slaptų, pasirinkite **Įkelti** **sertifikatą** ir įkelkite .cer sertifikato failą, kad jį būtų galima naudoti S2S autentifikavimui.
    ...
    7. SF išrašymo paslaugų komanda turi suteikti programai prieigą prie paslaugos. Siųsti šių parametrų vertes <D365EInvoiceSupport@microsoft.com> į:
...
1. ["Azure"](https://portal.azure.com) portale, eikite **į** virtualias mašinas ir **pasirinkite Kurti** naują.
...
12. Naujame VM eikite į **Priskirtą** \> **tapatybės sistemą** ir nustatykite **būsenos** pasirinktį **Į**.
13. Suteikti VM prieigą prie rakto saugyklos.

    1. Rakto "Vault" eikite į **prieigos valdymo (IAM)** \> **vaidmenų** priskyrimus.
    2. Pasirinkite **Įtraukti vaidmens** priskyrimą ir tada nustatykite šiuos laukus:
...
14. ["Azure"](https://portal.azure.com) portale eikite **į viešus IP adresus ir pasirinkite** VM sukurtą IP adresą.
...
2. Atidarykite vietinio kompiuterio sertifikato įfiksavimo įrankį. Daugiau informacijos rasite [Kaip: peržiūrėti sertifikatus, kai galima prispausti](/dotnet/framework/wcf/feature-details/how-to-view-certificates-with-the-mmc-snap-in) MMC.
3. Importuokite **caentrate.cer gamybos sertifikatą ir** **CAEntratetest.cer, kad jį būtų galima patikrinti**[patikimos šakninio sertifikavimo institucijos](/dotnet/framework/wcf/feature-details/working-with-certificates#certificate-stores) parduotuvėje. **(CAEntratetest.cer** yra šakninis CA sertifikatas, kurį pateikė institucija.)
...
3. Sukonfigūruokite tarnybą.
    ...
    3. Faile **web.config** suraskite šią eilutę ir pridėkite tarpinio serverio sertifikato nykščio atspaudą.
        ...
        > [!TIP]
        > Sistemai perėjus į gamybą, galite pakeisti rinkmenos web.config vertes, kad būtų sumažintas surinktos žurnalo informacijos kiekis ir diske būtų vietos diske. Mazge **\<system.diagnostics\>\<source\>** pakeiskite switchValue vertę **į** **Kritinė,** Klaida. Daugiau informacijos ieškokite [MS Service Trace](/dotnet/framework/wcf/service-trace-viewer-tool-svctraceviewer-exe) Viewer.
...
7. Pasirinkite ir sulaikykite (arba spustelėkite dešiniuoju pelės **mygtuku)** svetaines, tada pasirinkite **Įtraukti** svetainę.
...
19. Bet kurioje žiniatinklio naršyklėje eikite į **serverDNS/TrasmissioneFatture.svc.** Turi pasirodyti standartinis aptarnavimo puslapis.
...
20. Norėdami saugoti žurnalus ir failus, kurkite šiuos aplankus:
    ...
    - **C: \\ žurnalai \\** – parduotuvės žurnalo failai čia. Šiuos failus galite peržiūrėti [MS tarnybos sekimo peržiūros](/dotnet/framework/wcf/service-trace-viewer-tool-svctraceviewer-exe) programa.
    - **C: \\ failai – čia saugoti visus \\** atsakymo failus.
