...
> [!NOTE]
> Naudojant "Commerce 10.0.8" ir daugiau, "Retail Server" vadinamas "Commerce Scale Unit". Šiame straipsnyje taikoma keliose ankstesnėse programos versijose, *todėl "Retail Server* " naudojamas šiame straipsnyje.
>
> Kai kurie šiame straipsnyje nurodytų procedūrų veiksmai skiriasi, atsižvelgiant į naudojate "Commerce" versiją. Daugiau informacijos rasite Kas [naujo arba pakeista Dynamics 365 Retail](../get-started/whats-new.md).

### <a name="using-certificate-profiles-in-commerce-channels"></a>Sertifikatų profilių naudojimas "Commerce" kanaluose

"Commerce" 10.0.15 ir vėlesnėse versijose galite naudoti vartotojo nustatytus sertifikatų šablonus mažmeninės prekybos parduotuvėms [funkcijai, kuri palaiko atjungties režimą,](./certificate-profiles-for-retail-stores.md) kai rakto Vault arba Commerce Headquarters naudoti negalima. Ši funkcija išplečia mažmeninės prekybos kanalų [funkcijos paslapčių valdymas](../dev-itpro/manage-secrets.md).

Norėdami taikyti šią funkciją plėtinyje CRT, atlikite šiuos veiksmus.
...
5. Kurti išplėtimo CRT projektą.
...
Plėtinio CRT komponentai įtraukiami į pavyzdžius CRT. Norėdami atlikti šias procedūras, atidarykite sprendimą CRT CommerceRuntimeSamples.sln **,** dalyje **RetailSdk\\ SampleExtensions\\ CommerceRuntime**.
...
3. Nukopijuokite surinkimo failą į plėtinių CRT aplanką:
    ...
    - **" CRT Modern POS" vieta: kopijuokite** surinkimą **\\ į iš išorės** aplanką, esantį vietinio kliento CRT brokerio vietoje.
...
# <a name="application-update-4"></a>[Programos 4 naujinimas](#tab/app-update-4)
...
5. Nukopijuokite failus į plėtinių CRT aplanką:

    - **Parduotuvės serveris:** kopijuokite surinkimą į **\\\\ talpyklos išorės** aplanką, esantį IIS "Retail Server" svetainės vietoje.
    - **" CRT Modern POS" vieta: kopijuokite** surinkimą **\\ į iš išorės** aplanką, esantį vietinio kliento CRT brokerio vietoje.

6. Rasti plėtinio konfigūracijos failą, kuris skirtas CRT:

    - **Parduotuvės serveris:** failo vardas yra commerceruntime.ext.config **, jis yra talpyklos išoriniame aplanke,** **\\ kuris yra IIS "Retail Server" svetainėje.**
    - **" CRT Modern POS" vietinė:** **failo vardas yra CommerceRuntime.MPOSOffline.Ext.config**, jis priklauso vietinio kliento CRT brokerio vietai.

7. Užregistruokite CRT pakeitimą plėtinio konfigūracijos faile.
...
6. Užregistruokite "Retail Server" plėtinių priklausomybes.
    ...
    3. Užregistruokite CRT pakeitimą plėtinio konfigūracijos faile CRT. Šio failo vardas yra **commerceruntime.ext.config**, **jis** yra IIS mažmeninės prekybos serverio svetainės vietoje, talpyklos aplanke.
...
Ši dalis lygi "Retail Server" valdikliui, bet išplečia vietinį CRT, kuris naudojamas, kai kliento programa neprijungta.
...
4. Išspręskite **Proxies.RetailProxy** projekto priklausomybes:
    ...
    Atidarykite RetailSDK Proxies **RetailProxy\\ Proxies.RetailProxy.csproj, įtraukite RetailSDK\\ SampleExtensions\\ CommerceRuntime** extensions.SalesTransactionSimediaSample **CommerceRuntime.Extensions.SalesTransactionSitraukimoSample \\\\ projektą ir įtraukite projekto nuorodą į RetailProxy \\\\ projektą, kad nuoroda SalesTransactionSi readtureSample**.**·** **·**
...
4. Užregistruokite "Retail" tarpinio serverio keitimą plėtinio konfigūracijos faile. Failas pavadintas **RetailProxy.MPOSOffline.ext.config**, jis yra vietinio kliento brokerio CRT vietoje.
...
1. Atidarykite sprendimą RetailSdk **\\ POS\\ ModernPOS.sln** ir įsitikinkite, kad jį galima sukompiliuoti be klaidų. Be to, įsitikinkite, kad naudodami komandą Vykdyti galėsite paleisti "Modern POS" iš Visual Studio **"Microsoft**".
...
Daugiau informacijos ieškokite Norvegijos grynųjų [pinigų registro funkcijos](./emea-nor-cash-registers.md).
...
1. Atlikite veiksmus skyriuje "[Cloud POS" plėtinio komponentai arba "](#cloud-pos-extension-components)[Modern POS" plėtinio](#modern-pos-extension-components) komponentai šiame straipsnyje anksčiau.
...
4. Modifikuokite šiuos failus norėdami įtraukti išteklių failus Norvegijai į diegiamus paketus:
    - \_ Pakuotės SharedPackagingProjectComponents\Sdk.ModernPos.Shared.csproj
  ...
  - Failui **Sdk.ModernPos.Shared.csproj** 
    - Įtraukti eilutę į **ItemGroup** skyrių
        ...
        ```
    > [!Note]
    > Vietoje <File_name> failo pavadinimą. Tas pats taikoma ir kitiems toliau pateiktiems pavyzdžiui.
...
5. Modifikuokite sertifikato konfigūracijos failą nurodydami nykščio atspaudą, parduotuvės vietą ir sertifikato, kuris turi būti naudojamas pardavimo operacijoms pasirašyti, pavadinimą. Tada nukopijuokite konfigūracijos failą į nuorodų **aplanką**.
    ...
    **Failas pavadintas Contoso.Commerce.Runtime.SalesTransactionSitraukimoSample.dll.config**, **jis nurodytas dalyje CommerceRuntime\\ Extensions.SalesTransactionSirunple bin Debug\\\\**.
...
8. Pritaikykite paketus per Microsoft Dynamics ciklo tarnybas (LCS) arba rankiniu būdu. Daugiau informacijos ieškokite Create [deployable packages](../dev-itpro/retail-sdk/retail-sdk-packaging.md).
