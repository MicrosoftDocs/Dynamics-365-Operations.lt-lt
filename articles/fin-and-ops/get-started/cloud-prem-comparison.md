---
title: "Debesies ir vietinės versijų funkcijų palyginimas"
description: "Temoje parodyta, kurios funkcijos palaikomos debesies ir vietinėje versijose."
author: robinarh
manager: AnnBe
ms.date: 07/13/2017
ms.topic: article
ms.prod: 
ms.service: dynamics-ax-platform
ms.technology: 
audience: Application User, Developer, IT Pro
ms.reviewer: robinr
ms.search.scope: Operations
ms.custom: 89563
ms.assetid: 
ms.search.region: Global
ms.author: robinr
ms.search.validFrom: 2017-07-13
ms.dyn365.ops.version: Platform update 9
ms.translationtype: HT
ms.sourcegitcommit: 7e0a5d044133b917a3eb9386773205218e5c1b40
ms.openlocfilehash: e590f9adcf8992704f2733f1d970176d35eb123a
ms.contentlocale: lt-lt
ms.lasthandoff: 09/29/2017

---

# <a name="cloud-and-on-premises-feature-comparison"></a>Debesies ir vietinės versijų funkcijų palyginimas

Šioje temoje parodyta, ar kiekvieną funkciją galima naudoti tolesniuose produktuose.

+ „Dynamics 365 for Finance and Operations, Enterprise edition“ (debesies versija)
+ „Dynamics 365 for Finance and Operations, Enterprise edition“ (vietinė versija)

Tolesnėje lentelėje išvardytos produktų sritys. Nurodomas visos funkcijos palaikymas debesies ir vietinėje versijose. Kai konkrečios funkcijos skiriasi nuo bendrosios srities, jos išvardytos atskiroje stulpelio Funkcija eilutėje.

## <a name="dynamics-365-for-finance-and-operations"></a>„Dynamics 365 for Finance and Operations‟

| Sritis | Funkcija | Debesis | Vietoje|
|---|---|---|---|
| Atitiktis ir sertifikavimai | | Taip | Taip |
| | SOC 1 1 tipo sertifikavimas| Taip | Nr. |
| Kaštų apskaita | | Taip | Taip |
| | „Power BI“ kaštų apskaitos turinio paketas| Taip | Nr. |
| | Kaštų apskaitos darbo sritis mobiliajai programai| Taip | Nr. |
| Kaštų valdymas| | Taip | Taip |
| | „Power BI“ kaštų valdymo turinio paketas | Taip | Nr. |
| Duomenų valdymas ir integravimas| | Taip | Taip |
| | Konfigūracijomis paremtas plėtinys| Taip | Nr. |
| | Duomenų eksportavimas į savo duomenų sandėlį| Taip | Nr. |
| | Papildančiųjų naujinimų eksportavimo į duomenų objektą funkcijos įjungimas | Taip | Nr. |
| Dokumentų tvarkymas| | Taip| Taip |
| Finansų valdymas|  | Taip | Taip |
| Žinynas| | Taip | Nr. |
| Personalas | | Taip | Taip |
| Įžvalgos| | Taip | Taip |
| | Elektroninės ataskaitos (ER) – integravimas su LCS | Taip | Nr.|
| | Elektroninės ataskaitos (ER) – integravimas su „SharePoint“ | Taip | Nr.|
| | Integravimas su PowerBI.com| Taip | Nr. |
| | Analizės darbo sritys | Taip | Nr. |
| | Sumanaus verslo procesas: rekomendacijos| Taip | Nr. |
| Atsargų valdymas| | Taip | Taip |
| „Lifecycle Services“| | Taip | Taip |
| | Konfigūruojamieji verslo procesai | Taip | Nr. |
| Lokalizacijos| | Taip | Taip |
| Gamyba| | Taip | Taip |
| Bendrasis planavimas ir prognozavimas| | Taip | Taip |
| Mobilioji programa, darbo sritys ir platforma  | | Taip | Taip |
| „Office“ integravimas| | Taip | Taip |
| Organizacijos administravimas | | Taip | Taip |
| Payroll|  | Taip | Taip |
| | Tiesioginis depozitas| Taip | Nr. |
| Paraiškos| | Taip | Taip |
| | Išėjimo iš pirkimo paraiškos į išorinį katalogą žymėjimas| Taip | Nr. |
| | Pirkimo išlaidų analizės „Power BI“ ataskaitos| Taip | Nr. |
| Produkto informacijos valdymas | | Taip | Taip |
| Bendrojo produkto duomenys| | Taip | Taip|
| Gamyba | | Taip | Taip |
| | Gamybos našumo „Power BI“ ataskaitos| Taip | Nr. |
| Projektų valdymas ir apskaita|  | Taip | Taip |
| Pardavimas| | Taip | Taip |
| | Pardavimo ir pelningumo našumo „Power BI“ ataskaitos| Taip | Nr. |
| Sauga | | Taip | Taip |
| Aptarnavimo valdymas| | Taip | Taip |
| Tiekimo grandinės valdymas| | Taip | Taip |
| Transportavimo valdymas| | Taip | Taip |
| Tiekėjo bendradarbiavimas| | Taip | Taip|
| Sandėlio valdymas| | Taip | Taip |
| | Mobilioji sandėlio programa| Taip | Nr. |
| | Sandėliavimo „Power BI“ ataskaitos| Taip | Nr. |
| Žiniatinklio klientas | | Taip | Taip |
| | Užduočių įrašymo priemonė – užduočių įrašų įrašymas ar įkėlimas iš BPM bibliotekos| Taip | Nr. |
| Palaikymas | | Taip | Taip |
| | Prieiga prie palaikymo per meniu Pagalba ir palaikymas| Taip | Nr. |

### <a name="development-and-administration"></a>Programavimas ir administravimas

| Sritis | Funkcija | Debesis | Vietoje|
|---|---|---|---|
| Komponavimas ir tikrinimas | | Taip | Taip |
| Išplečiamumas|  | Taip | Taip |
| Stebėjimas ir telemetrija  || Taip | Taip |
| Platformų suderinamumas  || Taip | Taip |
| Aptarnavimas | | Taip | Taip |
| | Paslaugų teikimo aplinka | Taip | Nr. |
| Atnaujinti | | Taip | Taip |
| | Atnaujinti | Taip | Nr. |
| | Ankstesnių versijų plėtotė ir palaikymas | Taip | Nr. |
| „Visual Studio“ programavimas |  | Taip | Taip |


## <a name="dynamics-365-for-retail"></a>„Dynamics 365 for Retail‟ 

| Sritis | Funkcija | Debesis | Vietoje|
|---|---|---|---|
| Visos „Retail“ sritys | Visos „Retail“ funkcijos | Taip | Nr. |

## <a name="dynamics-365-for-talent"></a>„Dynamics 365 for Talent‟

| Sritis | Funkcija | Debesis | Vietoje|
|---|---|---|---|
| Visos „Talent“ sritys | Visos „Talent“ funkcijos | Taip | Nr. |




