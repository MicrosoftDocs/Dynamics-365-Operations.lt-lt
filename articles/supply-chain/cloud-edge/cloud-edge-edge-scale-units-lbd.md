...
Kraštų skalės vienetai veikia vietinėse aplinkose, todėl infrastruktūros reikalavimai panašus. Tačiau yra tam tikrų skirtumų, kuriuos reikia nurodyti:

- Kraštų skalės vienetai naudoja finansines ataskaitas, todėl jiems nereikia finansinių ataskaitų mazgų.
- Gamybos ir sandėliavimo darbo krūviai nereikia daug dirbti, todėl nagrinėkime galimybę atitinkamai įvertinti AOS mazgų gamybos galią.
...
1. Baigę duomenų bazių konfigūravimo veiksmą [, vykdykite toliau nurodytą scenarijų, norėdami sukonfigūruoti skalės](../../fin-ops-core/dev-itpro/deployment/setup-deploy-on-premises-pu41.md#configuredb) vieneto konvertavimo instrumentatoriaus duomenų bazę.
    ...
    > [!NOTE]
    > Nekonfigūruokite finansinių ataskaitų duomenų bazės atliekant duomenų [bazės konfigūravimo veiksmą](../../fin-ops-core/dev-itpro/deployment/setup-deploy-on-premises-pu41.md#configuredb).
...
1. Įdiegę aplinką, atlikite šiuos veiksmus:
    ...
    1. Patikrinkite, ar jūsų verslo duomenų bazėje (AXDB) įgalintas keitimų sekimas.
        ...
        1. Pasirinkite ir sulaikykite (arba spustelėkite dešiniuoju pelės mygtuku) savo verslo duomenų bazę (AXDB) ir pasirinkite **Ypatybės**.
    ...
    1. Pridėkite AD FS programos ID, kurį sukūrėte anksčiau (naudodami create-ADFSServerApplicationForEdgeScaleUnits.ps1 scenarijų) Azure AD į programų lentelę savo skalės vienete. Šį veiksmą galite atlikti rankiniu būdu naudodami vartotojo sąsają (vartotojo SĄSAJĄ). Taip pat galite užbaigti ją per duomenų bazę naudodami šį scenarijų.
...
1. Įdiegus aplinką, sukurkite papildomą programą, kuri įgalina Azure AD patikimą ryšį tarp jūsų centro ir svarstyklių vieneto.
...
1. Kai sukuriate programą, turite sukurti kliento slaptą ir išsaugoti informaciją "Azure" rakto saugykloje. Be to Azure AD, turite suteikti prieigą prie sukurtos programos, kad ji galėtų nuskaityti rakto saugykloje saugomus paslapius. Jūsų patogumui toliau pateikiamas scenarijus automatiškai atliks visus reikalingus veiksmus.
...
1. Įtraukite Azure AD ką tik sukurtą programos ID (naudodami scenarijų Create-SgToHubAADApplication.ps1) Azure AD į programų lentelę savo centre. Šį veiksmą galite atlikti naudodami vartotojo sąsają.
...
Konfigūruojate ir valdote savo kraštų skalės vienetą naudodami svarstyklių valdymo portalą. Daugiau informacijos ieškokite Skyriuje Valdyti skalės [vienetus ir darbo krūvius naudojant skalės vieneto tvarkytuvo portalą](./cloud-edge-landing-page.md#scale-unit-manager-portal).
