...
Norėdami gauti daugiau informacijos apie tai, kaip nustatyti "Azure" mašinos mokymosi paslaugą ir tada naudoti čia nustatytus parametrus, kad prie jos prisijungtų, žr. "Azure" mašinos mokymosi [paslaugos](#setup-amls) skyriaus nustatymas.
...
- **Trūkstamos vertės pakaitalai** – Nurodo, kaip užpildomos praeities duomenų spragos. Pasirinktys: *(skaitINĖ VERTĖ)*, *VIDUTINIS*, *ANKSTESNIS*, *INTERALALIZINIS,* *INTERUATINIS POLINENOMIJA*.
...
Galite nepaisyti šių parametrų verčių nueidami į Bendrojo **planavimo \> nustatymą \> Poreikio prognozės \> poreikio prognozės** parametrai. Poreikio **prognozės** parametrų puslapyje parametrų galite nepaisyti šiais būdais:
...
1. GitHub, atidarykite poreikio prognozės naudojant "Azure" mašinos mokymosi saugyklą (repo) šablonus ir [Dynamics 365 Supply Chain Management](https://github.com/microsoft/Dynamics-365-Supply-Chain-Management-Demand-Forecasting-With-Azure-Machine-Learning-Service) atsisiųskite šiuos failus:
...
1. Baikite vedlio veiksmus, naudodami ekrane pateiktas instrukcijas. Dirbę atmeskite šiuos taškus:
    ...
    - Būtinai pasirinkite geografinį regioną, kuris atitinka regioną, kuriame įdiegtas tiekimo grandinės valdymo egzempliorius. Kitu atveju kai kurie jūsų duomenys gali peržengia regiono ribas. Daugiau informacijos ieškokite toliau šioje [temoje](#privacy) pateiktame privatumo pranešime.
...
Daugiau informacijos ieškokite [Darbo srities](/azure/machine-learning/quickstart-create-resources#create-the-workspace) kūrimas.
...
1. GitHub, atidarykite poreikio prognozės naudojant "Azure" mašinos mokymosi repo šablonus ir atsisiųskite [Dynamics 365 Supply Chain Management](https://github.com/microsoft/Dynamics-365-Supply-Chain-Management-Demand-Forecasting-With-Azure-Machine-Learning-Service)**sampleInput.csv** failą.
1. Atidarykite saugojimo abonementą, kurį sukūrėte [1 žingsnyje: sukurkite naują darbo](#create-workspace) sritį.
1. Norėdami sukurti [konteinerį,](/azure/storage/blobs/storage-quickstart-blobs-portal#create-a-container) kuris pavadintas *"demplan-azureml", vadovaukitės instrukcijomis, pateiktomis svetainėje sukurkite konteinerį.*
...
1. GitHub, atidarykite poreikio [prognozės Dynamics 365 Supply Chain Management naudojant "Azure" įrenginio mokymosi](https://github.com/microsoft/Dynamics-365-Supply-Chain-Management-Demand-Forecasting-With-Azure-Machine-Learning-Service) repo šablonus ir atsisiųskite šiuos failus:
...
1. Prisiregistruokite prie "Azure" portalo.
1. Registruokite naują prašymą nuomininkui Azure Active Directory Azure AD (). Norėdami gauti daugiau [instrukcijų, žr. portalą, kad Azure AD sukurtumėte programą ir aptarnavimo pagrindinį puslapį, galiį pasiekti](/azure/active-directory/develop/howto-create-service-principal-portal) išteklius.
1. Pabaigę vedlį vadovaukitės ekrane pateikiamomis instrukcijomis. Naudoti numatytuosius parametrus.
1. Suteikite savo naujai Active Directory programai prieigą prie toliau nurodytų išteklių, kuriuos sukūrėte ["Azure" skyriuje nustatyti mašinų](#ml-workspace) mokymą. Instrukcijų žr.["Azure" vaidmenų priskyrimas naudojant "Azure"](/azure/role-based-access-control/role-assignments-portal?tabs=current) portalą. Šis veiksmas įgalins sistemą importuoti ir eksportuoti prognozavimo duomenis ir aktyvinti pardavimo galimybių paleidimą iš tiekimo grandinės valdymo.
...
1. Sertifikatuose **>** pasamdysite sukurtos programos skyrių, sukurkite programos slapyme. Instrukcijų ieškokite Kliento [slapto failo](/azure/active-directory/develop/quickstart-register-app#add-a-client-secret) pridėjimas.
...
1. Skirtuke **"Azure Machine Learning** Service" nustatykite šiuos laukus:
    ...
    - **Tarnybos pagrindinis programos ID** – įveskite programos, kurią sukūrėte Active Directory programos [skyriuje,](#aad-app) ID. Ši vertė naudojama norint įgalioti API užklausas "Azure" mašinos mokymosi tarnybai.
