...
description: "Pažangaus banko suderinimo funkcija suteikia galimybę importuoti elektroninius banko išrašus ir automatiškai juos suderinti su banko operacijomis programoje „Microsoft Dynamics 365 for Operations“. Šiame straipsnyje paaiškinama, kaip nustatyti banko išrašų importavimo funkciją."
author: twheeloc
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
Pažangaus banko suderinimo funkcija suteikia galimybę importuoti elektroninius banko išrašus ir automatiškai juos suderinti su banko operacijomis programoje „Microsoft Dynamics 365 for Operations“. Šiame straipsnyje paaiškinama, kaip nustatyti banko išrašų importavimo funkciją. 

Banko išrašo importavimo nustatymas priklauso nuo elektroninio banko išrašo formato. Microsoft Dynamics 365 operacijoms palaiko trys banko pareiškimas formatų out of the box: ISO20022 MT940 ir BAI2.

## <a name="sample-files"></a>Failų pavyzdžiai
Visi trys formatų, turite failų, kurie išversti elektroninio banko išrašą iš originalaus formato į formatą, kurį Dynamics 365 operacijoms gali naudoti. Reikiamus šaltinio failus galite rasti „Microsoft Visual Studio“ programų naršyklės mazge **Ištekliai**. Suradę failus, nukopijuokite juos į vieną žinomą vietą, kad sąrankos proceso metu galėtumėte juos lengvai nusiųsti.

| Išteklių pavadinimas                                           | Failo vardas                            |
|---------------------------------------------------------|--------------------------------------|
| BankStmtImport\_BAI2CSV\_į\_BAI2XML\_xslt              | BAI2CSV-to-BAI2XML.xslt              |
| BankStmtImport\_BAI2XML\_į\_susitaikymo\_xslt       | BAI2XML-to-Reconciliation.xslt       |
| BankStmtImport\_BankReconciliation\_į\_kompozicinių\_xslt | BankReconciliation-to-Composite.xslt |
...
| BankStmtImport\_MT940XML\_į\_susitaikymo\_xslt      | MT940XML-to-Reconciliation.xslt      |
...
Toliau pateikiami pavyzdžiai išsamiau banko susitaikymo importo failo techninį išdėstymą apibrėžimai ir trimis susijusiomis banko pareiškimo pavyzdys failus: https://mbs.microsoft.com/customersource/northamerica/AX/learning/documentation/how-to-articles/exofbankstfotechlayouts  
...
1.  Eikite į **darbo**&gt;**duomenų valdymo**.
2.  Click **Import**.
...
10. 1 eilės numeryje spustelėkite **Nusiųsti failą** ir pasirinkite failą** ISO20022XML-to-Reconciliation.xslt**, kurį įrašėte anksčiau. **Pastaba:** Dynamics 365 operacijų transformacijos failams sukurti standartinį formatą. Nes bankai dažnai nukrypsta nuo šiuo formatu, turite atnaujinti transformacijos failą susieti savo banko pareiškimo forma. <!-- For details about the expected format for ISO20022, see [Dynamics AX ISO20022 Layout](./media/dynamicsaxiso20022layout1.xlsx).-->
11. Click **New**.
...
1.  Eikite į **pinigų ir banko valdymo**&gt;**nustatymo**&gt;**banko susitaikymo sudėtingiau**&gt;**banko ataskaita formatas**.
2.  Click **New**.
...
1.  Eikite į **pinigų ir banko valdymo**&gt;**banko sąskaitos**.
...
12. 2 eilės numeryje spustelėkite **Nusiųsti failą** ir pasirinkite failą** MT940XML-to-Reconciliation.xslt**, kurį įrašėte anksčiau. **Pastaba:** Dynamics 365 operacijų transformacijos failams sukurti standartinį formatą. Nes bankai dažnai nukrypsta nuo šiuo formatu, turite atnaujinti transformacijos failą susieti savo banko pareiškimo forma. <!--- For details about the expected format for MT940, see [Dynamics AX MT940 Layout](./media/dynamicsaxmt940layout1.xlsx)-->
13. Click **New**.
...
11. Spustelėkite **Naujas**.
12. 2 eilės numeryje spustelėkite **Nusiųsti failą** ir pasirinkite failą** BAI2XML-to-Reconciliation.xslt**, kurį įrašėte anksčiau. **Pastaba:** Dynamics 365 operacijų transformacijos failams sukurti standartinį formatą. Nes bankai dažnai nukrypsta nuo šiuo formatu, ir jums gali tekti atnaujinti transformacijos failą susieti savo banko pareiškimo forma. <!--- For details about the expected format for BAI2, see [Dynamics AX BAI2 Layout](./media/dynamicsaxbai2layout1.xlsx).-->
13. Click **New**.
14. 3 eilės numeryje spustelėkite **Nusiųsti failą** ir pasirinkite failą **BankReconciliation-to-Composite.xslt**, kurį įrašėte anksčiau.
15. Spustelėkite **Taikyti pakeitimus**.
